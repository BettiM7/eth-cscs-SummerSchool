...
Understand the motivations behind using GPUs for HPC. Key architectural distinctions between CPU and GPU. Get started with connecting with the HPC cluster at CSCS.

`01_introduction.pdf`

`02_porting.pdf` along with setup.

Python HPC lectures will continue after.
...
Learn the programming model, common GPU libraries, and understand GPU memory management with practical exercises.
...
Learn using cooperating thread blocks for more advanced kernels. Understand concepts such as race conditions, thread synchronization, and atomics with practical exercises.
...
Understand implementing a real-world numerical simulation using a toy mini-app. Leverage previous concepts to implement working GPU code and compare it with a CPU version. The same example would be extended for future lessons on OpenACC as well.
...
#### NOTE: Solutions would be uploaded at the end of the day in the same repo in the `solutions/` folder.
